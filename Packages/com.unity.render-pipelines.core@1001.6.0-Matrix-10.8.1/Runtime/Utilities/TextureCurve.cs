...
namespace UnityEngine.Rendering
{
    ...
    public class TextureCurve : IDisposable
    {
        ...
        //PicoVideo;Basic;YangFan;Begin
        private bool m_CustomHashCodeDirty = false;
        
        public int GetCustomHashCode()
        {
            int hash = 17;
            if (!m_CustomHashCodeDirty)
            {
                return hash;
            }
            m_CustomHashCodeDirty = false;
            
            if (m_Curve != null)
            {
#if UNITY_EDITOR
                // 编辑器下支持拖动时动态刷新方便美术拉条调效果
                hash = hash * 23 + m_Curve.keys.GetHashCode();
#else
                hash = hash * 23 + m_Curve.GetHashCode();
#endif
            }

            hash = hash * 23 + m_Loop.GetHashCode();
            hash = hash * 23 + m_ZeroValue.GetHashCode();
            hash = hash * 23 + m_Range.GetHashCode();
            return hash;
        }
        //PicoVideo;Basic;YangFan;End
    }
    ...
    public class TextureCurveParameter : VolumeParameter<TextureCurve>
    {
        ...
        public int GetCustomHashCode()
        {
            ...
            return m_Value.GetCustomHashCode();
        }
        //PicoVideo;Basic;YangFan;End
    }
}
