...
#define UNITY_INV_PI      0.31830988618379067154
...
void SampleDirectionalLightmap(TEXTURE2D_LIGHTMAP_PARAM(lightmapTex, lightmapSampler), TEXTURE2D_LIGHTMAP_PARAM(lightmapDirTex, lightmapDirSampler), LIGHTMAP_EXTRA_ARGS, float4 transform,
    float3 normalWS, float3 viewDir, float smoothness, float specControl, float3 backNormalWS, bool encodedLightmap, real4 decodeInstructions, inout real3 bakeDiffuseLighting, inout real3 backBakeDiffuseLighting)
#else
void SampleDirectionalLightmap(TEXTURE2D_LIGHTMAP_PARAM(lightmapTex, lightmapSampler), TEXTURE2D_LIGHTMAP_PARAM(lightmapDirTex, lightmapDirSampler), LIGHTMAP_EXTRA_ARGS, float4 transform,
...
//PicoVideo;LightMode;Ernst;End
{
    ...
    //PicoVideo;LightMode;Ernst;Begin
    #if defined(_MATRIX_SPC_LIGHTINGMAP)
    float3 lightDir = direction.xyz * 2 - 1;
    float3 halfDir = normalize(normalize(lightDir) + viewDir);
    float perceptualRoughness = 1 - smoothness;
    float roughness = perceptualRoughness * perceptualRoughness;
    float spec = GGXTerm(max(dot(normalWS, halfDir), 0.0), perceptualRoughness);
    //float spec = pow(max(dot(normalWS, halfDir), 0.0), roughness);
    bakeDiffuseLighting += illuminance * spec * specControl;
    #endif
    //PicoVideo;LightMode;Ernst;End
...
real3 SampleDirectionalLightmap(TEXTURE2D_LIGHTMAP_PARAM(lightmapTex, lightmapSampler), TEXTURE2D_LIGHTMAP_PARAM(lightmapDirTex, lightmapDirSampler), LIGHTMAP_EXTRA_ARGS, float4 transform,
    float3 normalWS, float3 viewWS, float smoothness, float specControl, bool encodedLightmap, real4 decodeInstructions)
#else
real3 SampleDirectionalLightmap(TEXTURE2D_LIGHTMAP_PARAM(lightmapTex, lightmapSampler), TEXTURE2D_LIGHTMAP_PARAM(lightmapDirTex, lightmapDirSampler), LIGHTMAP_EXTRA_ARGS, float4 transform,
...
//PicoVideo;LightMode;Ernst;End
{
    ...
    SampleDirectionalLightmap(TEXTURE2D_LIGHTMAP_ARGS(lightmapTex, lightmapSampler), TEXTURE2D_LIGHTMAP_ARGS(lightmapDirTex, lightmapDirSampler), LIGHTMAP_EXTRA_ARGS_USE, transform,
                                    normalWS, viewWS, smoothness, specControl, backNormalWSUnused, encodedLightmap, decodeInstructions, bakeDiffuseLighting, backBakeDiffuseLightingUnused);
    #else
    SampleDirectionalLightmap(TEXTURE2D_LIGHTMAP_ARGS(lightmapTex, lightmapSampler), TEXTURE2D_LIGHTMAP_ARGS(lightmapDirTex, lightmapDirSampler), LIGHTMAP_EXTRA_ARGS_USE, transform,
