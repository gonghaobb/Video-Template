...
struct v2f
{
    ...
    float4 posObjectSpaceNonSkewed : TEXCOORD8;
    #if VLB_OCCLUSION_CLIPPING_PLANE
    float4 posWorldSpace : TEXCOORD1;
    #endif

#if !OPTIM_VS || !OPTIM_VS_FRESNEL_VS
    float4 posViewSpace_extraData : TEXCOORD2;
#endif
#if VLB_NOISE_3D || !OPTIM_VS || DEBUG_BLEND_INSIDE_OUTSIDE || !OPTIM_VS_FRESNEL_VS
...
inline float ComputeOutsideBeam(float3 posObjectSpace, float3 vecCamToPixOSN)
{
    // Compute normal
...
float ComputeFresnel(float3 posObjectSpace, float3 vecCamToPixOSN, float outsideBeam, float boostFactor)
{
    ...
    fresnel = lerp(boostFresnel, fresnel, 0); // no boosted fresnel if outside
...
inline float GetTiltDistanceFactor(float3 posObjectSpace)
{
    ...
    const float pixDistFromSource = abs(posObjectSpace.z);
    return pixDistFromSource + posObjectSpace.x * tiltVector.x + posObjectSpace.y * tiltVector.y;
}

float3 ComputeVertexFog(float distance, float4 color)
{
    ...
    fogFactor = real(saturate(distance * unity_FogParams.z + unity_FogParams.w));
    #elif defined(FOG_EXP) || defined(FOG_EXP2)
    // factor = exp(-(density*z)^2)
    // -density * z computed at vertex
    fogFactor = real(unity_FogParams.x * distance);
    #endif
    
    color.rgb = MixFogColor(color.rgb, fogColor.rgb, fogFactor);
    return color;
    #else
    return color.rgb;
    #endif
...
v2f vertShared(vlb_appdata v, float outsideBeam)
{
    ...
    vertexOS.xy *= lerp(coneRadius.x, coneRadius.y, vertexOS.z);

    // float3 scaleObjectSpace = float3(maxRadius, maxRadius, VLB_GET_PROP(_DistanceFallOff).z); // maxGeometryDistance
    float3 scaleObjectSpace = float3(coneRadius.zz, VLB_GET_PROP(_DistanceFallOff).z); // maxGeometryDistance

    o.posObjectSpaceNonSkewed.xyz = vertexOS.xyz * scaleObjectSpace;

    //mesh 为custom时，用于自定义mesh
#if VLB_MESH_SKEWING
    // skew the real vertex position
    vertexOS = SkewVectorOS(vertexOS, scaleObjectSpace);
#endif

    float4 posWorldSpace = VLBObjectToWorldPos(vertexOS);
...
#if VLB_OCCLUSION_CLIPPING_PLANE
    o.posWorldSpace = posWorldSpace;
#endif

#if defined(VLBWorldToViewPos)
    float3 posViewSpace = VLBWorldToViewPos(posWorldSpace.xyz);
    ...
    //     o.posObjectSpaceNonSkewed.w = (DistanceToPlane(posWorldSpace.xyz, addClippingPlaneWS.xyz, addClippingPlaneWS.w));
    // }
...
#if VLB_NOISE_3D
    o.uvwNoise_intensity.rgb = Noise3D_GetUVW(posWorldSpace.xyz, o.posObjectSpace);
#endif
    ...
    outsideBeam = ComputeOutsideBeam(o.posObjectSpaceNonSkewed.xyz, vecCamToPixOSN);
    ...
    intensity *= ComputeFresnel(o.posObjectSpaceNonSkewed.xyz, vecCamToPixOSN, outsideBeam, boostFactor);
#endif
    ...
    // 雾效计算移到vertex阶段
     o.color.rgb =  ComputeVertexFog(posViewSpace.z, o.color);
    
    float extraData = 0;
#else
    float extraData = isCap;
    // o.uvwNoise_intensity.a = 1;
     // o.color.rgb =  1;
#endif // OPTIM_VS
...
half4 fragShared(v2f i)
{
    ...
    // additional clipping plane
    // 移到Vertex
    // clip(i.posObjectSpaceNonSkewed.w);
    // DYNAMIC OCCLUSION
#if VLB_OCCLUSION_CLIPPING_PLANE
    {
...
#if !OPTIM_VS || !OPTIM_VS_FRESNEL_VS || DEBUG_BLEND_INSIDE_OUTSIDE
    float outsideBeam = i.cameraPosObjectSpace_outsideBeam.w;
#endif
