...
namespace VLB
{
    ...
    public class BeamGeometry : MonoBehaviour, MaterialModifier.Interface
    {
        ...
        /// </summary>
        public void RegenerateMesh(int id = 0, bool isDoubleSide = true)
        {
            ...
            switch (m_Master.geomMeshType)
            {
                case MeshType.Custom:
                    {
                        coneMesh = MeshGenerator.GenerateConeZ_Radius(1f, 1f, 1f, m_Master.geomCustomSides, m_Master.geomCustomSegments, m_Master.geomCap, false);
                ...
                case MeshType.LOD:
                    {
                        coneMesh = MeshGenerator.GenerateConeZ_Radius(1f, 1f, 1f, m_Master.geomLODSettings[id].geomSides, m_Master.geomLODSettings[id].geomSegments, m_Master.geomCap, false);
                        coneMesh.hideFlags = Consts.Internal.ProceduralObjectsHideFlags;
                        meshFilter.mesh = coneMesh;
                        break;
                    }
            ...
            UpdateMaterialAndBounds(isDoubleSide);
        }
...
#pragma warning restore 0162
        ...
        void SetMaterialKeywords(string shaderKeyword, bool enable)
        {
            if (m_CustomMaterial)
            {
                m_CustomMaterial.SetKeywordEnabled(shaderKeyword, enable);
            }
        }
        ...
        public void SetDynamicOcclusionCallback(string shaderKeyword, MaterialModifier.Callback cb)
        {
            ...
            if (m_CustomMaterial)
            {
                ...
                UpdateMaterialAndBounds(false);
        }

        public void UpdateMaterialIfChanged(VolumetricLightBeam.ChangeFlag changeFlag)
        {
            MaterialChangeStart();
            if (CheckFlag(changeFlag, VolumetricLightBeam.ChangeFlag.color))
            {
                UpdateMaterialColor();
            }

            if (CheckFlag(changeFlag, VolumetricLightBeam.ChangeFlag.intensity))
            {
                UpdateMaterialIntensity();
            }

            if (CheckFlag(changeFlag, VolumetricLightBeam.ChangeFlag.spotAngle))
            {
                UpdateMaterialSpotAngle();
            }

            if (CheckFlag(changeFlag, VolumetricLightBeam.ChangeFlag.fallOffEnd))
            {
                UpdateMaterialFallOffEnd();
            }
            MaterialChangeStop();
        }

        private bool CheckFlag(VolumetricLightBeam.ChangeFlag changeFlag, VolumetricLightBeam.ChangeFlag checkFlag)
        {
            return ((uint)changeFlag & (uint)checkFlag) > 0;
        }

        private void UpdateMaterialFallOffEnd()
        {
        ...
        private void UpdateMaterialSpotAngle()
        {
            ...
            float maxRadius = Mathf.Max(coneRadius.x, coneRadius.y);
            float normalizedRadiusStart = coneRadius.x / maxRadius;
            float normalizedRadiusEnd = coneRadius.y / maxRadius;
            SetMaterialProp(ShaderProperties.ConeRadius, new Vector4(normalizedRadiusStart, normalizedRadiusEnd, maxRadius));
        ...
        public void UpdateMaterialAndBounds(bool isDoubleSide)
        {
            Debug.Assert(m_Master);
            ...
            MaterialChangeStart();
            {
                ...
                SetMaterialProp(ShaderProperties.Cull, isDoubleSide ? (int)UnityEngine.Rendering.CullMode.Off : (int)UnityEngine.Rendering.CullMode.Back);
                ...
                {
                    SetMaterialKeywords(ShaderKeywords.DepthBlend,
                        m_Master.depthBlendDistance > 0 &&
                        m_Master.additionalCameraData != null && m_Master.additionalCameraData.requiresDepthTexture
                    );
                }
