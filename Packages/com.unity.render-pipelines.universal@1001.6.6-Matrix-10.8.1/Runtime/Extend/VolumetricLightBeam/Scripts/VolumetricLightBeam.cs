...
namespace VLB
{
    ...
    public partial class VolumetricLightBeam : MonoBehaviour
    {
        ...
        //URP
        private UniversalAdditionalCameraData m_AdditionalCameraData = null;
        ...
        public UniversalAdditionalCameraData additionalCameraData
        {
            get
            {
                if (m_AdditionalCameraData == null)
                {
                    if (Camera.main)
                    {
                        m_AdditionalCameraData = Camera.main.GetComponent<UniversalAdditionalCameraData>();
                    }
                }

                return m_AdditionalCameraData;
            }
        }
        ...
        private Color oldColor = Consts.Beam.FlatColor;
        /// <summary>
        /// Gradient color applied along the beam, if colorMode is Gradient (takes account of the color and alpha variations).
        /// </summary>
        public Gradient colorGradient = Consts.Beam.GradientColor;
        private Gradient oldColorGradient = Consts.Beam.GradientColor;
#if UNITY_EDITOR
        ...
        public bool enableDoubleSide = true;
        ...
        private float oldIntensityInside = Consts.Beam.IntensityDefault;
        ...
        private float oldIntensityOutside = Consts.Beam.IntensityDefault;
        ...
        private float oldSpotAngle = Consts.Beam.SpotAngleDefault;
        ...
        [Serializable]
        public class LODGeomSettings
        {
            [Min(3)]
            public int geomSides = Consts.Beam.GeomSidesDefault;
            public int geomSegments = Consts.Beam.GeomSegmentsDefault;
            [Range(0,1f)]
            public float screenRelativeTransitionHeight = 0;
        }

        public List<LODGeomSettings> geomLODSettings = new List<LODGeomSettings>()
        {
            new LODGeomSettings(){geomSegments = Consts.Beam.geomSettingsLOD0Default.geomSegments, geomSides = Consts.Beam.geomSettingsLOD0Default.geomSides, screenRelativeTransitionHeight = Consts.Beam.geomSettingsLOD0Default.screenRelativeTransitionHeight },
            new LODGeomSettings(){geomSegments = Consts.Beam.geomSettingsLOD1Default.geomSegments, geomSides = Consts.Beam.geomSettingsLOD1Default.geomSides, screenRelativeTransitionHeight = Consts.Beam.geomSettingsLOD1Default.screenRelativeTransitionHeight },
            new LODGeomSettings(){geomSegments = Consts.Beam.geomSettingsLOD2Default.geomSegments, geomSides = Consts.Beam.geomSettingsLOD2Default.geomSides, screenRelativeTransitionHeight = Consts.Beam.geomSettingsLOD2Default.screenRelativeTransitionHeight },
            new LODGeomSettings(){geomSegments = Consts.Beam.geomSettingsLOD3Default.geomSegments, geomSides = Consts.Beam.geomSettingsLOD3Default.geomSides, screenRelativeTransitionHeight = Consts.Beam.geomSettingsLOD3Default.screenRelativeTransitionHeight },
            new LODGeomSettings(){geomSegments = Consts.Beam.geomSettingsLOD4Default.geomSegments, geomSides = Consts.Beam.geomSettingsLOD4Default.geomSides, screenRelativeTransitionHeight = Consts.Beam.geomSettingsLOD4Default.screenRelativeTransitionHeight },
        };
        ...
        private float oldFallOffEnd = Consts.Beam.FallOffEnd;
...
#pragma warning restore 0414
        ...
        private List<BeamGeometry> m_BeamGoemLODList = new List<BeamGeometry>();
        ...
        public virtual void GenerateGeometry()
        {
            ...
            if (geomMeshType != MeshType.LOD)
            {
                ...
                m_BeamGeom.RegenerateMesh(0, enableDoubleSide);
                m_BeamGeom.visible = enabled;
            }
            else
            {
                if (geomLODSettings == null)
                {
                    ResetGeomLODSettings();
                }
        ...
        {
            if (geomLODSettings.Count == 0)
            {
                return;
            }
                
            if (m_BeamGoemLODList == null)
            {
                m_BeamGoemLODList = new List<BeamGeometry>();
            }
            
            lodGroup = Utils.NewWithComponent<LODGroup>("Beam Geometry LODGroup");
            var transform1 = lodGroup.transform;
            transform1.parent = this.transform;
            transform1.localPosition = Vector3.zero;
            transform1.localScale = Vector3.one;
            transform1.localRotation = Quaternion.identity;
            lodGroup.gameObject.hideFlags = Consts.Internal.ProceduralObjectsHideFlags;
            
            lodList.Clear();
            float minTransitionHeight = 1;
            for (int i = 0; i < geomLODSettings.Count; i++)
            {
                BeamGeometry beamGeometry = Utils.NewWithComponent<BeamGeometry>("Beam Geometry LOD" + i);
                beamGeometry.Initialize(this);
                // CallOnBeamGeometryInitializedCallback();
                    
                beamGeometry.RegenerateMesh(i, enableDoubleSide);
                beamGeometry.visible = enabled;
                beamGeometry.transform.SetParent(lodGroup.transform);
                m_BeamGoemLODList.Add(beamGeometry);
                
                LOD lod = new LOD();
                lod.renderers = beamGeometry.GetComponents<Renderer>();
                if (geomLODSettings[i].screenRelativeTransitionHeight >= minTransitionHeight)
                {
                    geomLODSettings[i].screenRelativeTransitionHeight = minTransitionHeight - 0.001f;
                }

                minTransitionHeight = geomLODSettings[i].screenRelativeTransitionHeight;
                lod.screenRelativeTransitionHeight = minTransitionHeight;
                if (minTransitionHeight >= 0)
                {
                    lodList.Add(lod);
                }
            }
        ...
        private void ResetGeomLODSettings()
        {
            geomLODSettings = new List<LODGeomSettings>()
            {
                new LODGeomSettings(){geomSegments = Consts.Beam.geomSettingsLOD0Default.geomSegments, geomSides = Consts.Beam.geomSettingsLOD0Default.geomSides, screenRelativeTransitionHeight = Consts.Beam.geomSettingsLOD0Default.screenRelativeTransitionHeight },
                new LODGeomSettings(){geomSegments = Consts.Beam.geomSettingsLOD1Default.geomSegments, geomSides = Consts.Beam.geomSettingsLOD1Default.geomSides, screenRelativeTransitionHeight = Consts.Beam.geomSettingsLOD1Default.screenRelativeTransitionHeight },
                new LODGeomSettings(){geomSegments = Consts.Beam.geomSettingsLOD2Default.geomSegments, geomSides = Consts.Beam.geomSettingsLOD2Default.geomSides, screenRelativeTransitionHeight = Consts.Beam.geomSettingsLOD2Default.screenRelativeTransitionHeight },
        ...
        public virtual void UpdateAfterManualPropertyChange()
        {
            ...
            if (geomMeshType != MeshType.LOD)
            {
                if (m_BeamGeom)
                {
                    m_BeamGeom.UpdateMaterialAndBounds(enableDoubleSide);
                }
            }
            else
            {
                for (int i = 0; i < m_BeamGoemLODList.Count; ++i)
                {
                    if (m_BeamGoemLODList[i])
                    {
                        m_BeamGoemLODList[i].UpdateMaterialAndBounds(enableDoubleSide);
                    }
                }
            }
        }
        
        private void CheckChangeFlag()
        {
            uint changeFlag = (uint)ChangeFlag.none;
            if (CheckIfColorChanged())
            {
                changeFlag |= (uint)ChangeFlag.color;
            }

            if (CheckIfFloatParameterChanged(fallOffEnd, ref oldFallOffEnd))
            {
                changeFlag |= (uint)ChangeFlag.fallOffEnd;
            }
            
            if (CheckIfFloatParameterChanged(intensityInside, ref oldIntensityInside))
            {
                changeFlag |= (uint)ChangeFlag.intensity;
            }
            
            if (CheckIfFloatParameterChanged(intensityOutside, ref oldIntensityOutside))
            {
                changeFlag |= (uint)ChangeFlag.intensity;
            }
            
            if (CheckIfFloatParameterChanged(spotAngle, ref oldSpotAngle))
            {
                changeFlag |= (uint)ChangeFlag.spotAngle;
            }
            ...
            if (changeFlag > 0)
            {
                if (geomMeshType != MeshType.LOD)
                {
                    if (m_BeamGeom)
                    {
                        m_BeamGeom.UpdateMaterialIfChanged((ChangeFlag)changeFlag);
                    }
                }
                else
                {
                    for (int i = 0; i < m_BeamGoemLODList.Count; ++i)
                    {
                        if (m_BeamGoemLODList[i])
                        {
                            m_BeamGoemLODList[i].UpdateMaterialIfChanged((ChangeFlag)changeFlag);
                        }
                    }
                }
            }
        ...
        public bool CheckIfFloatParameterChanged(float checkValue, ref float oldValue)
        {
            bool result = !oldValue.Equals(checkValue);
            oldValue = checkValue;
            return result;
        }

        private bool CheckIfColorChanged()
        {
            bool result = false;
            if (colorMode == ColorMode.Flat)
            {
                result = !oldColor.Equals(color);
                oldColor = color;
            }
            else
            {
                result = !oldColorGradient.Equals(colorGradient);
                oldColorGradient = colorGradient;
            }
        ...
        // {
        //     if (Application.isPlaying && m_CoPlaytimeUpdate == null)
        //     {
        ...
        void DestroyBeam()
        {
            ...
            for (int i = 0; i < m_BeamGoemLODList.Count; ++i)
            {
                if (m_BeamGoemLODList[i])
                {
                    DestroyImmediate(m_BeamGoemLODList[i].gameObject);
                    m_BeamGoemLODList[i] = null;
                }
            }
            m_BeamGoemLODList.Clear();
            if (lodGroup)
            {
