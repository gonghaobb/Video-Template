...
namespace UnityEngine.Rendering.Universal.Internal
{
    ...
    public class ColorGradingLutPass : ScriptableRenderPass
    {
        ...
        private int m_ColorGradingLutParametersHash = 17;
        private RenderTexture m_LutTexture = null;
        
        public RenderTexture GetInternalLutTexture(ref RenderingData renderingData)
        {
            if (m_LutTexture == null)
            {
                ...
                {
                    vrUsage = VRTextureUsage.None // We only need one for both eyes in VR
                };
                m_LutTexture = new RenderTexture(desc)
                {
        ...
        public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            using (new ProfilingScope(cmd, ProfilingSampler.Get(URPProfileId.ColorGradingLUT)))
            {
                ...
                int parametersHash = 17;
                parametersHash = parametersHash * 23 + channelMixer.GetHashCode();
                parametersHash = parametersHash * 23 + colorAdjustments.GetHashCode();
                parametersHash = parametersHash * 23 + curves.GetCustomHashCode();
                parametersHash = parametersHash * 23 + liftGammaGain.GetHashCode();
                parametersHash = parametersHash * 23 + shadowsMidtonesHighlights.GetHashCode();
                parametersHash = parametersHash * 23 + splitToning.GetHashCode();
                parametersHash = parametersHash * 23 + tonemapping.GetHashCode();
                parametersHash = parametersHash * 23 + whiteBalance.GetHashCode();
                
                if (17 == m_ColorGradingLutParametersHash || parametersHash != m_ColorGradingLutParametersHash)
                {
                    m_ColorGradingLutParametersHash = parametersHash;
                    ...
                    // Render the lut
                    Blit(cmd, GetInternalLutTexture(ref renderingData), GetInternalLutTexture(ref renderingData),
                        material);
        ...
        //PicoVideo;Basic;YangFan;Begin
        private void SafeReleaseLutTexture()
        {
            if (m_LutTexture != null)
            {
