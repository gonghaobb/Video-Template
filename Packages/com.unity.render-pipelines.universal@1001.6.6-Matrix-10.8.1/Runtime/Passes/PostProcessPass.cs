...
namespace UnityEngine.Rendering.Universal.Internal
{
    ...
    public class PostProcessPass : ScriptableRenderPass
    {
        ...
        private RenderTexture m_InternalLutTexture = null;
        ...
        //PicoVideo;FSR;ZhengLingFeng;Begin
        //PicoVideo;Basic;YangFan;Begin
        public void Setup(in RenderTextureDescriptor baseDescriptor, in RenderTargetHandle source, in RenderTargetHandle destination, in RenderTargetHandle depth, in RenderTargetHandle internalLut, bool hasFinalPass, bool enableSRGBConversion, bool hasExternalPostPasses, RenderTexture internalLutTexture = null)
        //PicoVideo;Basic;YangFan;End
        {
            ...
            //PicoVideo;Basic;YangFan;Begin
            m_InternalLutTexture = internalLutTexture;
            //PicoVideo;Basic;YangFan;End
        }

        public void SetupFinalPass(in RenderTargetHandle source, bool hasExternalPostPasses = true)
        {
            m_Source = source;
        ...
        private new void Blit(CommandBuffer cmd, int source, RenderTargetIdentifier destination, Material material, int passIndex = 0)
        {
            cmd.SetGlobalTexture(UniversalRenderPipeline.IsEnableFoveatedRenderingAndSubsampledLayout(source) ? ShaderPropertyId.subsampledLayoutSourceTex : ShaderPropertyId.sourceTex, source);
        ...
        //PicoVideo;FoveatedFeature;YangFan;Begin
        private void Blit(CommandBuffer cmd, int source, RenderTargetIdentifier destination, Material material, int passIndex, bool needstencil, Camera camera)
        {
            cmd.SetGlobalTexture(UniversalRenderPipeline.IsEnableFoveatedRenderingAndSubsampledLayout(source) ? ShaderPropertyId.subsampledLayoutSourceTex : ShaderPropertyId.sourceTex, source);
        //PicoVideo;FoveatedFeature;YangFan;End
            if (m_UseDrawProcedural)
            {
                ...
                cmd.SetGlobalVector(ShaderPropertyId.scaleBias, scaleBias);
                cmd.SetRenderTarget(new RenderTargetIdentifier(destination, 0, CubemapFace.Unknown, -1),
                    RenderBufferLoadAction.Load, RenderBufferStoreAction.Store, new RenderTargetIdentifier(m_Depth.id,0,CubemapFace.Unknown,-1),RenderBufferLoadAction.Load, RenderBufferStoreAction.DontCare);
        ...
        void Render(CommandBuffer cmd, ref RenderingData renderingData)
        {
            ...
            using (new ProfilingScope(cmd, ProfilingSampler.Get(URPProfileId.UberPostProcess)))
            {
                ...
                bool doEarlyFsrColorConversion = (!m_hasExternalPostPasses &&
                                                  (((cameraData.imageScalingMode == FSRUtils.ImageScalingMode.Upscaling) && (cameraData.upscalingFilter == FSRUtils.ImageUpscalingFilter.FSR)) &&
                ...
                if (UniversalRenderPipeline.IsEnableFoveatedRenderingAndSubsampledLayout(GetSource()))
                {
                    m_Materials.uber.EnableKeyword(ShaderKeywordStrings.UseFoveatedSubsampledLayout);
                    cmd.SetGlobalTexture(ShaderPropertyId.subsampledLayoutSourceTex, GetSource());
                }
                else
                {
                    m_Materials.uber.DisableKeyword(ShaderKeywordStrings.UseFoveatedSubsampledLayout);
        ...
        void DoMotionBlur(CameraData cameraData, CommandBuffer cmd, int source, int destination)
        {
            ...
            //PicoVideo;FoveatedFeature;YangFan;Begin
            CoreUtils.SetKeyword(material, ShaderKeywordStrings.UseFoveatedSubsampledLayout, UniversalRenderPipeline.IsEnableFoveatedRenderingAndSubsampledLayout(source));
            //PicoVideo;FoveatedFeature;YangFan;End
        ...
        void SetupBloom(CommandBuffer cmd, int source, Material uberMaterial, ref CameraData cameraData)
        {
            ...
            // if (m_Bloom.isCustomBloom.value)
            // {
            //     tw = 512;
            //     th = 512 * m_Descriptor.height / m_Descriptor.width;
            // }
        //PicoVideo;CustomBloom;ZhouShaoyang;End
            ...
            //PicoVideo;CustomBloom;ZhouShaoyang;Begin
            if (m_Bloom.iterationMode.value == BloomIterationMode.Skip)
            {
                iterations -= m_Bloom.skipIterations.value;
                
            }
            else if(m_Bloom.iterationMode.value == BloomIterationMode.Fixed)
            {
                iterations = m_Bloom.fixedIterations.value;
            }
            ...
            //PicoVideo;FoveatedFeature;YangFan;Begin
            CoreUtils.SetKeyword(bloomMaterial, ShaderKeywordStrings.UseFoveatedSubsampledLayout, UniversalRenderPipeline.IsEnableFoveatedRenderingAndSubsampledLayout(source));
            //PicoVideo;FoveatedFeature;YangFan;End

            //PicoVideo;CustomBloom;ZhouShaoyang;Begin
            // Prefilter
            RenderTextureDescriptor desc = GetCompatibleDescriptor(tw, th, m_DefaultHDRFormat);
            switch (m_Bloom.maskMode.value)
            {
                case BloomMaskMode.Off:
                    ...
                    break;
                case BloomMaskMode.DepthStencil_Forward:
                    desc = GetCompatibleDescriptor(m_Descriptor.width, m_Descriptor.height, m_DefaultHDRFormat);
                #if UNITY_EDITOR
                    desc.msaaSamples = cameraData.cameraTargetDescriptor.msaaSamples;
                #endif
                    // ConfigureTarget(colorAttachment, m_Depth.Identifier());
                    // CoreUtils.SetRenderTarget(cmd, colorAttachments, depthAttachment, clearFlag, clearColor);
                    cmd.GetTemporaryRT(ShaderConstants._BloomMipDown[0], desc, FilterMode.Bilinear);
                    cmd.GetTemporaryRT(ShaderConstants._BloomMipUp[0], desc, FilterMode.Bilinear);
                    Blit(cmd, source, ShaderConstants._BloomMipDown[0], bloomMaterial, 6, true, cameraData.camera);
                #if UNITY_EDITOR
                    desc.msaaSamples = 1;
                #endif
                    break;
                case BloomMaskMode.DepthStencil_Reverse:
                    desc = GetCompatibleDescriptor(m_Descriptor.width, m_Descriptor.height, m_DefaultHDRFormat);
                #if UNITY_EDITOR
                    desc.msaaSamples = cameraData.cameraTargetDescriptor.msaaSamples;
                #endif
                    ...
                    cmd.GetTemporaryRT(ShaderConstants._BloomMipDown[0], desc, FilterMode.Bilinear);
                    cmd.GetTemporaryRT(ShaderConstants._BloomMipUp[0], desc, FilterMode.Bilinear);
                    Blit(cmd, source, ShaderConstants._BloomMipDown[0], bloomMaterial, 7, true, cameraData.camera);
                #if UNITY_EDITOR
                    desc.msaaSamples = 1;
                #endif
                    break;
                case BloomMaskMode.Alpha_Forward:
                    cmd.GetTemporaryRT(ShaderConstants._BloomMipDown[0], desc, FilterMode.Bilinear);
                    cmd.GetTemporaryRT(ShaderConstants._BloomMipUp[0], desc, FilterMode.Bilinear);
                    Blit(cmd, source, ShaderConstants._BloomMipDown[0], bloomMaterial, 8);
                    break;
                case BloomMaskMode.Alpha_Reverse:
                    cmd.GetTemporaryRT(ShaderConstants._BloomMipDown[0], desc, FilterMode.Bilinear);
                    cmd.GetTemporaryRT(ShaderConstants._BloomMipUp[0], desc, FilterMode.Bilinear);
                    Blit(cmd, source, ShaderConstants._BloomMipDown[0], bloomMaterial, 9);
                    break;
            }
            //PicoVideo;CustomBloom;ZhouShaoyang;End
            ...
            for (int i = 1; i < mipCount; i++)
            {
                ...
                desc.width = tw;
                desc.height = th;
                int mipDown = -1, mipUp = -1;
                //PicoVideo;CustomBloom;ZhouShaoyang;Begin
                if (!m_Bloom.isCustomBloom.value)
                {
                    mipDown = ShaderConstants._BloomMipDown[i];
                    mipUp = ShaderConstants._BloomMipUp[i];
                ...
                else
                {
                    ...
                    cmd.GetTemporaryRT(mipDown, desc, FilterMode.Bilinear);
                    var descNext = desc;
                    descNext.width = Mathf.Max(1, tw >> 1);
                    descNext.height = Mathf.Max(1, tw >> 1);
                    cmd.GetTemporaryRT(ShaderConstants._BloomMipDown[i + 1], descNext, FilterMode.Bilinear);
                    cmd.GetTemporaryRT(ShaderConstants._BloomMipUp[i], desc, FilterMode.Bilinear);
            ...
            for (int i = mipCount - 2; i >= 0; i--)
            {
                ...
                {
                    Blit(cmd, highMip, BlitDstDiscardContent(cmd, dst), bloomMaterial, 5);
                }
                //PicoVideo;CustomBloom;ZhouShaoyang;End
            }
        ...
        void SetupChromaticAberration(Material material)
        {
            ...
            Vector2 uvOffset = m_ChromaticAberration.uvOffset.value;
            material.SetVector(ShaderConstants._Chroma_Params, new Vector3(m_ChromaticAberration.intensity.value * 0.05f, uvOffset.x, uvOffset.y));
        ...
        void SetupColorGrading(CommandBuffer cmd, ref RenderingData renderingData, Material material)
        {
            ...
            //PicoVideo;Basic;YangFan;Begin
            if (m_InternalLutTexture != null)
            {
                cmd.SetGlobalTexture(ShaderConstants._InternalLut, m_InternalLutTexture);
            }
            else
            {
        ...
        void RenderFinalPass(CommandBuffer cmd, ref RenderingData renderingData)
        {
            ...
            if (cameraData.imageScalingMode != FSRUtils.ImageScalingMode.None)
            {
                // FSR is only considered "enabled" when we're performing upscaling. (downscaling uses a linear filter unconditionally)
                bool isFsrEnabled = ((cameraData.imageScalingMode == FSRUtils.ImageScalingMode.Upscaling) && (cameraData.upscalingFilter == FSRUtils.ImageUpscalingFilter.FSR));
                ...
                if (isSetupRequired)
                {
                    ...
                    Blit(cmd, m_Source.Identifier(), ShaderConstants._ScalingSetupTexture, m_Materials.scalingSetup);

                    cmd.SetGlobalTexture(ShaderPropertyId.sourceTex, ShaderConstants._ScalingSetupTexture);

                    sourceRtId.id = ShaderConstants._ScalingSetupTexture;
                }

                switch (cameraData.imageScalingMode)
                {
                    case FSRUtils.ImageScalingMode.Upscaling:
                    {
                        ...
                        {
                            case FSRUtils.ImageUpscalingFilter.Point:
                            {
                                material.EnableKeyword(ShaderKeywordStrings.PointSampling);
                                break;
                            }

                            case FSRUtils.ImageUpscalingFilter.Linear:
                            {
                                // Do nothing as linear is the default filter in the shader
                                break;
                            }

                            case FSRUtils.ImageUpscalingFilter.FSR:
                            {
                                ...
                                Blit(cmd, sourceRtId.Identifier(), ShaderConstants._UpscaledTexture, m_Materials.easu);
                    ...
                    case FSRUtils.ImageScalingMode.Downscaling:
                    {
