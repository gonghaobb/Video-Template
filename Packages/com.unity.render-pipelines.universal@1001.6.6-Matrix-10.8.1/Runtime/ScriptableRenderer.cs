...
namespace UnityEngine.Rendering.Universal
{
    ...
    public abstract partial class ScriptableRenderer : IDisposable
    {
        ...
        private static Matrix4x4 mvViewProjMatrix = Matrix4x4.identity;
        ...
        public static void SetCameraMatrices(CommandBuffer cmd, ref CameraData cameraData, bool setInverseMatrices)
        {
            ...
            if (!cameraData.xr.enabled)
            {
                cmd.SetGlobalMatrix(ShaderPropertyId.prevViewAndProjectionMatrix, mvViewProjMatrix);
                mvViewProjMatrix = GL.GetGPUProjectionMatrix(projectionMatrix, true) * viewMatrix;
            }
            //PicoVideo;AppSW;Ernst;End
        ...
        void SetPerCameraShaderVariables(CommandBuffer cmd, ref CameraData cameraData)
        {
            ...
            float pixelWidth = cameraData.rtDynamicResolutionType != RTDynamicResolutionType.UseDynamicResolution ? (float)cameraData.pixelRect.width : (float)cameraData.camera.scaledPixelWidth;
            float pixelHeight = cameraData.rtDynamicResolutionType != RTDynamicResolutionType.UseDynamicResolution ? (float)cameraData.pixelRect.height : (float)cameraData.camera.scaledPixelHeight;
            float scaledCameraWidth = pixelWidth * cameraData.renderScale;
            float scaledCameraHeight = pixelHeight * cameraData.renderScale;
            float cameraWidth = pixelWidth;
            float cameraHeight = pixelHeight;
        ...
        public static RenderBufferStoreAction GetActiveDepthStoreActive()
        {
            return m_ActiveDepthStoreAction;
        }
        public static RenderBufferStoreAction GetActiveColorStoreAction(int i)
        {
            return m_ActiveColorStoreActions[i];
        }
        ...
        public ScriptableRenderer(ScriptableRendererData data)
        {
            ...
            if (UniversalRenderPipeline.asset)
            {
                ...
                if (UniversalRenderPipeline.asset.GetOptimizedRenderPipelineFeatureActivated(OptimizedRenderPipelineFeature.MemorylessMSAA))
                {
                    m_StoreActionsOptimizationSetting = StoreActionsOptimization.Discard;
                }
            }
        ...
        public void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            VFX.VFXManager.ProcessCameraCommand(camera, cmd);
                ...
                UniversalRenderPipeline.SetFrameBufferFoveatedRendering();
        ...
        void ExecuteRenderPass(ScriptableRenderContext context, ScriptableRenderPass renderPass, ref RenderingData renderingData)
        {
            ...
            {
                //PicoVideo;SubPass;Ernst;Begin
                renderPass.EarlyConfigure(context, ref renderingData);
                //PicoVideo;SubPass;Ernst;End
        ...
        void SetRenderPassAttachments(CommandBuffer cmd, ScriptableRenderPass renderPass, ref CameraData cameraData)
        {
                ...
                if (!RenderingUtils.SequenceEqual(renderPass.colorAttachments, m_ActiveColorAttachments) || renderPass.depthAttachment != m_ActiveDepthAttachment || finalClearFlag != ClearFlag.None)
                {
                    ...
                    if (lastValidRTindex >= 0)
                    {
                        ...
                        if (cameraData.xr.enabled)
                        {
                            ...
                            //PicoVideo;AppSW;Ernst;Begin
                            cameraData.xr.UpdateGPUViewAndProjectionMatrices(cmd, ref cameraData, !isRenderToBackBufferTarget,renderPass is Internal.MotionVectorPass);
                            //PicoVideo;AppSW;Ernst;End
                        }
                    #endif
                    }
            ...
            else
            {
                        ...
                        renderPass.colorStoreActions[0] != m_ActiveColorStoreActions[0] || renderPass.depthStoreAction != m_ActiveDepthStoreAction)
                {
                    ...
                    {
                        // Debug.Log($"Cannot set rendertarget among the subpass. => {cameraData.camera.name}, {renderPass.GetType().Name}, (1){passColorAttachment != m_ActiveColorAttachments[0]}:{passColorAttachment},{m_ActiveColorAttachments[0]};" +
                        //           $"(2){passDepthAttachment != m_ActiveDepthAttachment}:{passDepthAttachment},{m_ActiveDepthAttachment};(3){finalClearFlag != ClearFlag.None}:{finalClearFlag};" +
                        //           $"(4){renderPass.colorStoreActions[0] != m_ActiveColorStoreActions[0]}:{renderPass.colorStoreActions[0]},{m_ActiveColorStoreActions[0]};" +
                        //           $"(5){renderPass.depthStoreAction != m_ActiveDepthStoreAction}:{renderPass.depthStoreAction},{m_ActiveDepthStoreAction}");
                    }
                    else
                    {
                        // Debug.Log($"Cannot set rendertarget among the subpass. => {cameraData.camera.name}, {renderPass.GetType().Name}, (1){passColorAttachment != m_ActiveColorAttachments[0]}:{passColorAttachment},{m_ActiveColorAttachments[0]};" +
                    ...
                    if (cameraData.xr.enabled)
                    {
                        ...
                        //PicoVideo;AppSW;Ernst;End
                        cameraData.xr.UpdateGPUViewAndProjectionMatrices(cmd, ref cameraData, !isRenderToBackBufferTarget, renderPass is MotionVectorPass);
                    }
#endif
                }
            }
