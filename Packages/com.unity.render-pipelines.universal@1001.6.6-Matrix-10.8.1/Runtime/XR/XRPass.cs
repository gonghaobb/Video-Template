...
namespace UnityEngine.Rendering.Universal
{
    ...
    class XRPass
    {
        ...
        //PicoVideo;AppSW;Ernst;Begin
        internal RenderTargetIdentifier motionVectorRenderTarget = -1;
        ...
        internal static XRPass Create(XRDisplaySubsystem.XRRenderPass xrRenderPass, int multipassId, ScriptableCullingParameters cullingParameters, Material occlusionMeshMaterial)
        {
            ...
            if (xrRenderPass.hasMotionVectorPass)
            {
                ...
                RenderTextureDescriptor rtMotionVectorDesc = new RenderTextureDescriptor(xrMotionVectorDesc.width / downSampling, xrMotionVectorDesc.height / downSampling, xrMotionVectorDesc.colorFormat, xrMotionVectorDesc.depthBufferBits, xrMotionVectorDesc.mipCount);
        ...
        private Matrix4x4 mvViewProjMatrix = Matrix4x4.identity;
        //PicoVideo;AppSW;Ernst;End
        private Matrix4x4[] stereoCameraProjectionMatrix = new Matrix4x4[2];

        //PicoVideo;AppSW;Ernst;Begin
        internal void UpdateGPUViewAndProjectionMatrices(CommandBuffer cmd, ref CameraData cameraData, bool isRenderToTexture, bool useMotionVector = false)
        //PicoVideo;AppSW;Ernst;End
        {
            ...
            if (cameraData.xr.singlePassEnabled)
            {
                ...
                RenderingUtils.SetStereoViewAndProjectionMatrices(cmd, stereoViewMatrix, stereoProjectionMatrix, stereoCameraProjectionMatrix, true, prevViewValid, stereoPrevViewMatrix, useMotionVector);
                //PicoVideo;AppSW;Ernst;End
            ...
            else
            {
                ...
                {   
                    ...
                    mvViewProjMatrix = projectionMatrix *  cameraData.xr.GetViewMatrix(0);
                }
            }
            //PicoVideo;AppSW;Ernst;End
        }
