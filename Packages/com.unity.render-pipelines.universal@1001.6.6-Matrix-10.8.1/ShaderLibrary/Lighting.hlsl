...
half3 SampleLightmap(float2 lightmapUV, half3 normalWS, half smoothness, half specControl, half3 viewWS)
#else
...
//PicoVideo;LightMode;Ernst;End
{
    ...
    //PicoVideo;LightMode;Ernst;Begin
        ...
        return SampleDirectionalLightmap(TEXTURE2D_LIGHTMAP_ARGS(LIGHTMAP_NAME, LIGHTMAP_SAMPLER_NAME),
            ...
            LIGHTMAP_SAMPLE_EXTRA_ARGS, transformCoords, normalWS, viewWS, smoothness, specControl, encodedLightmap, decodeInstructions);
    #else
        return SampleDirectionalLightmap(TEXTURE2D_LIGHTMAP_ARGS(LIGHTMAP_NAME, LIGHTMAP_SAMPLER_NAME),
            TEXTURE2D_LIGHTMAP_ARGS(LIGHTMAP_INDIRECTION_NAME, LIGHTMAP_SAMPLER_NAME),
...
#if defined(LIGHTMAP_ON)
    #if defined(_MATRIX_SPC_LIGHTINGMAP)
        #define SAMPLE_GI(lmName, shName, normalWSName, smoothnessName, specControlName, viewWSName) SampleLightmap(lmName, normalWSName, smoothnessName, specControlName, viewWSName)
    #else
        #define SAMPLE_GI(lmName, shName, normalWSName) SampleLightmap(lmName, normalWSName)
    #endif
#else
    #if defined(_MATRIX_SPC_LIGHTINGMAP)
        #define SAMPLE_GI(lmName, shName, normalWSName, smoothnessName, specControlName, viewWSName) SampleSHPixel(shName, normalWSName)
    #else
...
//PicoVideo;BoxProjectedCubemapDirection;XiaoPengCheng;Begin
...
//PicoVideo;BoxProjectedCubemapDirection;XiaoPengCheng;End
...
//PicoVideo;CompatibleUnity2021;XiaoPengCheng;Begin
//half4 UniversalFragmentBlinnPhong(InputData inputData, half3 diffuse, half4 specularGloss, half smoothness, half3 emission, half alpha)
half4 UniversalFragmentBlinnPhongInternal(InputData inputData, half3 diffuse, half4 specularGloss, half smoothness, half3 emission, half alpha)
//PicoVideo;CompatibleUnity2021;XiaoPengCheng;End
{
    // To ensure backward compatibility we have to avoid using shadowMask input, as it is not present in older shaders
#if defined(SHADOWS_SHADOWMASK) && defined(LIGHTMAP_ON)
    half4 shadowMask = inputData.shadowMask;
...
//PicoVideo;CompatibleUnity2021;XiaoPengCheng;Begin
inline half4 UniversalFragmentBlinnPhong(InputData inputData, SurfaceData surfaceData)
{
    #if defined(_SPECGLOSSMAP) || defined(_SPECULAR_COLOR)
        surfaceData.smoothness = exp2(10 * surfaceData.smoothness + 1);
    #endif

    return UniversalFragmentBlinnPhongInternal(inputData, surfaceData.albedo * surfaceData.occlusion, half4(surfaceData.specular, 1.0f), surfaceData.smoothness, surfaceData.emission, surfaceData.alpha);
}

inline half4 UniversalFragmentBlinnPhong(InputData inputData, half3 diffuse, half4 specularGloss, half smoothness, half3 emission, half alpha)
{
    return UniversalFragmentBlinnPhongInternal(inputData, diffuse, specularGloss, smoothness, emission, alpha); 
}
//PicoVideo;CompatibleUnity2021;XiaoPengCheng;End
