...
namespace TMPro.EditorUtilities
{
    ...
    {
        static readonly uint[] IgnoreMissingChars = new uint[] { 9, 10, 13, 32 }; // \t \n \r \space

        // [LoveEngine] Mod 
        // https://bytedance.feishu.cn/docs/doccnvLlfN6RfxjyTxCKCbEDqTr#8YdgRg
        // please update the FeiShu document when you update this alogrithm/procedure below.
        public static IEnumerator AutoGenerateFontAsset(TMP_FontAsset fontAsset)
        {
            var settings = fontAsset.creationSettings;
            if(string.IsNullOrEmpty(settings.sourceFontFileGUID)
            || string.IsNullOrEmpty(settings.referencedTextAssetGUID)
            || settings.characterSetSelectionMode != 8)
            {
                throw new Exception("[LoveEngine] [TMP]: You should create a font asset manually by a characters files first.");
            }

            var font = AssetDatabase.LoadAssetAtPath<Font>(AssetDatabase.GUIDToAssetPath(settings.sourceFontFileGUID));
            if(font == null)
            {
                throw new Exception("[LoveEngine] [TMP]: Font File is invalid, you should create a font asset manually by a characters files.");
            }

            var characters = AssetDatabase.LoadAssetAtPath<TextAsset>(AssetDatabase.GUIDToAssetPath(settings.referencedTextAssetGUID));
            if(characters == null)
            {
                throw new Exception("[LoveEngine] [TMP]: Characters File is invalid, you should create a font asset manually by a characters files.");
            }

            var window = ShowFontAtlasCreatorWindow(fontAsset);
            window.m_autoGenerateMissingCharReport = null;
            window.m_autoGenerate = true;
            try
            {
                var time = Time.time;
                while(window.m_FontAtlasTexture == null && Time.time - time < 10) yield return null;

                if(window.m_FontAtlasTexture == null)
                {
                    throw new Exception("[LoveEngine] [TMP]: auto generate failed!");
                }

                if(!string.IsNullOrEmpty(window.m_autoGenerateMissingCharReport) )
                {
                    var checklist = window.m_MissingCharacters.Concat(window.m_ExcludedCharacters);

                    foreach(var c in checklist)
                    {
                        if(IgnoreMissingChars.Any(cc => cc == c))
                        {
                            continue;
                        }

                        throw new Exception("[LoveEngine] [TMP]: auto generate failed! " +
                            "Because of Missing(no font) or Excluded(texture size is too small) characters:\n" + 
                            window.m_autoGenerateMissingCharReport);
                    }
                }                

                window.m_autoGenerateSave = true;
                time = Time.time;
                while(window.m_autoGenerateSave && Time.time - time < 10) yield return null;

                if(window.m_autoGenerateSave)
                {
                    throw new Exception("[LoveEngine] [TMP]: auto generate failed!");
                }
            }
        ...
        string m_autoGenerateMissingCharReport = null;
        bool m_autoGenerate = false;
        bool m_autoGenerateSave = false;
        ...
        public static TMPro_FontAssetCreatorWindow ShowFontAtlasCreatorWindow(TMP_FontAsset fontAsset)
        {
            var window = GetWindow<TMPro_FontAssetCreatorWindow>();
            ...
            if (string.IsNullOrEmpty(fontAsset.creationSettings.sourceFontFileGUID) == false)
            {
                ...
                // [LoveEngine] Mod : we don't want to override the settings in font assets
        ...
        Dictionary<int, List<Glyph>> m_GlyphsToRender = new Dictionary<int, List<Glyph>>();
        ...
        // [LoveEngine] Mod: Add fallback fonts for generation 
        int m_FontFallbackCount;
        [SerializeField]
        Font[] m_FallbackFontFiles;
        private SerializedObject m_SerializedObject;
        private SerializedProperty m_FontFilesseSerializedProperty;
        ...
        public void OnEnable()
        {
            ...
            m_FontFilesseSerializedProperty = m_SerializedObject.FindProperty("m_FallbackFontFiles");
        }
        ...
        void DrawControls()
        {
            ...
            EditorGUI.BeginDisabledGroup(m_IsProcessing);
            {
                ...
                m_SourceFontFile = EditorGUILayout.ObjectField("Source Font", m_SourceFontFile, typeof(Font), false) as Font;
                ...
                EditorGUI.BeginChangeCheck();
                GUI.enabled = m_SourceFontFile != null;
                m_SourceFontFaceIndex = EditorGUILayout.Popup("Font Face", m_SourceFontFaceIndex, m_SourceFontFaces);
                m_SerializedObject.Update();
                EditorGUILayout.PropertyField(m_FontFilesseSerializedProperty);
                m_SerializedObject.ApplyModifiedProperties();
                m_FontFallbackCount = m_FontFilesseSerializedProperty.arraySize;
            ...
            if ((m_autoGenerate || GUILayout.Button("Generate Font Atlas")) && GUI.enabled)
            {
                if (m_autoGenerate) m_IsRepaintNeeded = true;
                ...
                if (!m_IsProcessing && m_SourceFontFile != null)
                {
                    ...
                    if (errorCode == FontEngineError.Success)
                    {
                        errorCode = FontEngine.LoadFontFace(m_SourceFontFile, 0, m_SourceFontFaceIndex);
                    ...
                    if (errorCode == FontEngineError.Success)
                    {
                        ...
                        // [LoveEngine] Mod: FontEngine.LoadFontFace is not available in a thread. Remove ThreadPool.QueueUserWorkItem invoking. 
                        ...
                        //ThreadPool.QueueUserWorkItem(PackGlyphs =>
                        {
                            ...
                            // [LoveEngine] Mod: A mapping from glyph index to font index. -1 stands for the default font.
                            Dictionary<uint, int> glyphMapping = new Dictionary<uint, int>();

                            // Check if requested characters are available in the source font file.
                            for (int i = 0; i < characterSet.Length; i++)
                            {
                                FontEngine.LoadFontFace(AssetDatabase.GetAssetPath(m_SourceFontFile));
                                ...
                                {
                                    // [LoveEngine] Mod: Check fallback fonts list.
                                    bool flag = false;
                                    for (int j = 0; j < m_FallbackFontFiles.Length; ++j)
                                    {
                                        var font = m_FallbackFontFiles[j];
                                        FontEngine.LoadFontFace(AssetDatabase.GetAssetPath(font));
                                        if (FontEngine.TryGetGlyphIndex(unicode, out glyphIndex))
                                        {
                                            if (m_CharacterLookupMap.ContainsKey(unicode))
                                            {
                            ...
                            if (m_AvailableGlyphsToAdd.Count > 0)
                            {
                                ...
                                if (m_PointSizeSamplingMode == 0) // Auto-Sizing Point Size Mode
                                {
                                    ...
                                    for (int iteration = 0; iteration < 15 && optimumPointSizeFound == false && m_PointSize > 0; iteration++)
                                    {
                                        ...
                                        // [LoveEngine] Mod: Check fallback fonts list.
                                        for (int i = -1; i < m_FontFallbackCount; ++i)
                                        {
                                            m_GlyphsToPack.Clear();
                                            if(i == -1)
                                            {
                                            ...
                                            {
                                                FontEngine.LoadFontFace(AssetDatabase.GetAssetPath(m_FallbackFontFiles[i]));
                                            }

                                            FontEngine.SetFaceSize(m_PointSize);

                                            for (int j = 0; j < m_AvailableGlyphsToAdd.Count; j++)
                                            {
                                                uint glyphIndex = m_AvailableGlyphsToAdd[j];
                                                if (!glyphMapping.ContainsKey(glyphIndex) || glyphMapping[glyphIndex] != i)
                                                {
                                                    continue;
                                                }
                                                Glyph glyph;
                            ...
                            foreach (Glyph glyph in m_GlyphsPacked)
                            {
                                ...
                                // [LoveEngine] Mod: Add glyphs into different list based on the font it uses.
                                // Add glyphs to list of glyphs that need to be rendered.
                                if (glyph.glyphRect.width > 0 && glyph.glyphRect.height > 0)
                                {
                                    int index = glyphMapping[glyph.index];
                                    if (!m_GlyphsToRender.ContainsKey(index))
                                    {
                                        m_GlyphsToRender.Add(index, new List<Glyph>());
                                    }

                                    m_GlyphsToRender[index].Add(glyph);
                                }
                        ...
                        //ThreadPool.QueueUserWorkItem(RenderGlyphs =>
                        {
                            if (m_IsGenerationCancelled == false)
                            {
                                ...
                                // [LoveEngine] Mod: Use different fonts to render. Check whether this font is used or not before rendering.
                                for(int i = -1; i < m_FallbackFontFiles.Length; ++i)
                                {
                                    if (!m_GlyphsToRender.ContainsKey(i))
                                    {
                                        continue;
                                    }
                                    var glyphsToRender = m_GlyphsToRender[i];

                                    // Render and add glyphs to the given atlas texture.
                                    FontEngineError err = FontEngineError.Success;
                                    if (glyphsToRender.Count > 0)
                                    {
                                        if (i == -1)
                                        {
                                            err = FontEngine.LoadFontFace(AssetDatabase.GetAssetPath(m_SourceFontFile));
                                            if (err != FontEngineError.Success)
                                            {
                                                throw new Exception("Can't load font " + AssetDatabase.GetAssetPath(m_SourceFontFile));
                                            }
                                        }
                                        else 
                                        {
                                            err = FontEngine.LoadFontFace(AssetDatabase.GetAssetPath(m_FallbackFontFiles[i]));
                                            if (err != FontEngineError.Success)
                                            {
                                                throw new Exception("Can't load font " + AssetDatabase.GetAssetPath(m_FallbackFontFiles[i]));
                                            }
                                        }

                                        FontEngine.SetFaceSize(m_PointSize);
                                        err = FontEngine.RenderGlyphsToTexture(glyphsToRender, m_Padding, m_GlyphRenderMode, m_AtlasTextureBuffer, m_AtlasWidth, m_AtlasHeight);
                                        if (err != FontEngineError.Success)
                                        {
                                            throw new Exception("Failed to render characters in " + AssetDatabase.GetAssetPath(m_FallbackFontFiles[i]));
                                        }
                                    }
                                }
            ...
            {
                EditorGUI.ProgressBar(progressRect, Mathf.Clamp01(m_AtlasGenerationProgress), m_AtlasGenerationProgressLabel);
            }            
            ...
            // [LoveEngine] Mod 
            if ((m_autoGenerateSave || GUILayout.Button("Save")) && GUI.enabled)
            {
                if (m_autoGenerateSave) m_IsRepaintNeeded = true;
                m_autoGenerateSave = false;
        ...
        string[] GetFontFaces()
        {
            FontEngine.LoadFontFace(m_SourceFontFile, 0, 0);
        ...
        void UpdateRenderFeedbackWindow()
        {
            ...
            m_autoGenerateMissingCharReport = missingGlyphReport;
        }
        ...
        FontAssetCreationSettings SaveFontCreationSettings()
        {
            ...
            settings.fallBackCount = m_FontFallbackCount;
            settings.fallBackFontList = new string[m_FontFallbackCount];
            for (int i = 0; i < m_FontFallbackCount; ++i)
            {
                settings.fallBackFontList[i] = AssetDatabase.AssetPathToGUID(AssetDatabase.GetAssetPath(m_FallbackFontFiles[i]));
            }
        ...
        void LoadFontCreationSettings(FontAssetCreationSettings settings)
        {
            ...
            m_FontFallbackCount = settings.fallBackCount;
            m_FallbackFontFiles = new Font[m_FontFallbackCount];
            for (int i = 0; i < m_FontFallbackCount; ++i)
            {
                m_FallbackFontFiles[i] = AssetDatabase.LoadAssetAtPath<Font>(AssetDatabase.GUIDToAssetPath(settings.fallBackFontList[i]));
            }
        }
