...
namespace TMPro
{
    ...
    public class TMP_FontAsset : TMP_Asset
    {
        ...
        /// Source font file when atlas population mode is set to dynamic. Null when the atlas population mode is set to static.
        /// </summary>
        public string sourceFontFilePath
        {
            get { return m_SourceFontFilePath; }
            internal set { m_SourceFontFilePath = value; }
        }
        [SerializeField]
        ...
        public static TMP_FontAsset CreateFontAsset(Font font, int faceIndex, int samplingPointSize, int atlasPadding, GlyphRenderMode renderMode, int atlasWidth, int atlasHeight, AtlasPopulationMode atlasPopulationMode = AtlasPopulationMode.Dynamic, bool enableMultiAtlasSupport = true)
        {
            // Load Font Face
        ...
        public static TMP_FontAsset CreateFontAssetInstance(Font font, int atlasPadding, GlyphRenderMode renderMode, int atlasWidth, int atlasHeight, AtlasPopulationMode atlasPopulationMode, bool enableMultiAtlasSupport)
        {
            ...
            if (((GlyphRasterModes)renderMode & GlyphRasterModes.RASTER_MODE_BITMAP) == GlyphRasterModes.RASTER_MODE_BITMAP)
            {
                ...
                var shader = TMP_Settings.ShaderRef_MobileBitmap; // deal.g use setting to store static ref shader
                if(shader == null) shader = ShaderUtilities.ShaderRef_MobileBitmap;
            ...
            else
            {
                ...
                // Optimize by adding static ref to shader.
                var shader = TMP_Settings.ShaderRef_MobileSDF; // deal.g use setting to store static ref shader
                if (shader == null) shader = ShaderUtilities.ShaderRef_MobileSDF;
        ...
        */
        ...
        internal bool TryAddCharacterInternal(uint unicode, out TMP_Character character)
        {
            ...
            // Make sure atlas texture is readable. @LoveEngine
            ...
            { // @LoveEngine
                Debug.LogWarning("Unable to add the requested character to font asset [" + this.name + "]'s atlas texture. Please make the texture [" + m_AtlasTextures[m_AtlasTextureIndex].name + "] readable or enable multi-atlas.", m_AtlasTextures[m_AtlasTextureIndex]);
            }
