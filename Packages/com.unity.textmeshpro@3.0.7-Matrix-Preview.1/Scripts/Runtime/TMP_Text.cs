...
namespace TMPro
{
    ...
    public enum TMP_TextElementType { Character, Sprite, Quad }; // deal.g add Quad enum
    ...
    public class TMP_QuadUtilities
    {

        public static string GetAttrValue(Dictionary<int, string> attrs, string attrName)
        {
            // TMP use Time33 hash algorithm in case insensitive
            var nameHash = TMP_TextParsingUtilities.GetHashCode(attrName);
            if (nameHash == 0) return null;

            string value;
            attrs.TryGetValue(nameHash, out value);
            return value;
        }
    }

    /// <summary>
    /// TextMeshPro Quad support, used in QuadCallback
    /// </summary>
    public class TMP_QuadTagInfo
    {
        public TMP_QuadTagInfo()
        {
        }
        public string getAttrValue(string attrName)
        {
            return TMP_QuadUtilities.GetAttrValue(attrs, attrName);
        }
        public Dictionary<int, string> attrs;
    }

    public interface ITMP_QuadEntity
    {
        int DefaultWidth();
        int DefaultHeight();
        void OnRefresh(TMP_QuadTagInfo info);
        void OnDestroy();
    }

    public delegate ITMP_QuadEntity QuadTagCallbackDelegate(GameObject quadObject, TMP_QuadTagInfo quadTagInfo);
    ...
    {
        #region Added by deal.g, for multi-language switch font mat
        ...
        #region Quad Attachment object support, by deal.g

        private bool m_IgnoreQuadScaleForVLayout = false;
        public bool ignoreQuadScaleForVLayout { get => m_IgnoreQuadScaleForVLayout; set => m_IgnoreQuadScaleForVLayout = value; }

        // Notes: deal.g
        // Quads are assigned in the F000 Private Area + quad Index
        // Sprites are assigned in the E000 Private Area + sprite Index
        protected readonly uint QUAD_CHARCODE_START = 0xF000;
        /// <summary>
        /// Quad Glyph support: {0,0,w,h} by deal.g
        /// </summary>
        protected GlyphRect m_quadRect;

        /// <summary>
        /// User should simple create entity for this tag
        /// </summary>
        protected static QuadTagCallbackDelegate m_quadTagCallback;
        /// <summary>
        /// Sets quad tag callback
        /// </summary>
        public static QuadTagCallbackDelegate QuadTagCallback { get => m_quadTagCallback; set => m_quadTagCallback = value; }

        protected TMP_QuadRoot m_quadRoot; // The quadRoot to manage quad objects

        protected List<TMP_QuadItem> m_quadItems;
        protected int m_quadIndex = 0;

        protected List<TMP_QuadCharacter> m_quadCharacters; // clear at function: ClearSubMeshObjects

        TMP_QuadTagInfo m_curQuadTagInfo = null;

        /// <summary>
        /// Destroy unused quad entities and notify user
        /// </summary>
        /// <param name="usedQuadCount"></param>
        protected void DestroyUnusedQuadEntities(int usedQuadCount)
        {
            // Can't do cleanup unused quad at rebuild layout time, otherwise will trigger exception
            if (CanvasUpdateRegistry.IsRebuildingLayout()) return;

            if (m_quadCharacters == null) return;
            for (int unused = usedQuadCount; unused < m_quadCharacters.Count; ++unused)
            {
                var quadChar = m_quadCharacters[unused];
                if (quadChar != null)
                    DestroyQuadEntity(quadChar.quadItem);
            }
        }


        /// <summary>
        /// Track QuadRoot, call at Awake
        /// </summary>
        protected void TrackQuadRoot()
        {
            TMP_QuadRoot quadRoot = GetComponentInChildren<TMP_QuadRoot>();
            if (quadRoot != null)
            {
                quadRoot.Owner = this;

                if (m_quadItems == null) m_quadItems = new List<TMP_QuadItem>();
                if (m_quadCharacters == null) m_quadCharacters = new List<TMP_QuadCharacter>();
                m_quadItems.Clear();
                m_quadCharacters.Clear();

                var quadItems = quadRoot.GetComponentsInChildren<TMP_QuadItem>(true);
                foreach(var quadItem in quadItems)
                {
                    quadItem.Owner = quadRoot;
                    m_quadItems.Add(quadItem);
                    m_quadCharacters.Add(null); // place holder only
                }
        ...
        protected TMP_QuadRoot GetQuadRoot()
        {
            if (m_quadRoot != null)
                return m_quadRoot;
            m_quadRoot = TMP_QuadRoot.CreateQuadRoot(this);
            return m_quadRoot;
        }

        // Cached

        protected Vector2Int RefreshQuadEntity(ITMP_QuadEntity entity, TMP_QuadTagInfo info)
        {
            entity.OnRefresh(info);
            return new Vector2Int(entity.DefaultWidth(), entity.DefaultHeight());
        }
        protected TMP_QuadCharacter GetRefreshedQuadCharacter(int index)
        {
            var quadRoot = GetQuadRoot();

            if (m_quadItems == null) m_quadItems = new List<TMP_QuadItem>();
            if (m_quadCharacters == null) m_quadCharacters = new List<TMP_QuadCharacter>();

            TMP_QuadCharacter quadChar = null;

            // this function must call stride = 1, so simple append object if not exists
            if (index < m_quadCharacters.Count)
            {
                quadChar = m_quadCharacters[index];
            }
            else /* if (index == m_quadObjects.Count)*/
            {
                m_quadCharacters.Add(null); // place holder
                m_quadItems.Add(null);
            }

            if (quadChar == null)
            { // quad element invalid, recreate it
                quadChar = new TMP_QuadCharacter((uint)(QUAD_CHARCODE_START + index));
                m_quadCharacters[index] = quadChar;
            }

            quadChar.UpdateAttributes(m_htmlTag, m_xmlAttribute);

            // m_quadRect.y; // for sprite the y is offset of spritesheet

            quadChar.quadItem = m_quadItems[index]; // Set quadObject from object sources
            if (quadChar.quadItem == null)
            { // quadElement.quadGlyph.quadObject invalid recreate it
                var quadItem = TMP_QuadItem.CreateQuadItem(String.Format("Quad_{0}", index + 1), quadRoot);
                quadChar.quadItem = quadItem;
                m_quadItems[index] = quadItem; // update quadItem at index
            }

            // set pivot for display quad ypos same with TMP sprite
            (quadChar.quadItem.transform as RectTransform).pivot = new Vector2(0.5f, 0);

            Vector2Int defaultExtent = new Vector2Int(128, 128);

            if (m_quadTagCallback != null)
            {
                if (m_curQuadTagInfo == null) m_curQuadTagInfo = new TMP_QuadTagInfo();
                m_curQuadTagInfo.attrs = quadChar.attrs;

                var entity = quadChar.quadItem.Entity;
                if (entity == null)
                {
                    entity = m_quadTagCallback(quadChar.quadItem.gameObject, m_curQuadTagInfo);
                    if (entity != null)
                    { // quad metrics reset by calback
                        quadChar.quadItem.Entity = entity;

                        defaultExtent = RefreshQuadEntity(entity, m_curQuadTagInfo);
                    }
                    else
                        DestroyQuadEntity(quadChar.quadItem);
                }
            ...
            m_quadRect.width = quadChar.width;
            m_quadRect.height = quadChar.height;

            if (m_quadRect.width == 0) m_quadRect.width = defaultExtent.x;
            if (m_quadRect.height == 0) m_quadRect.height = defaultExtent.y;

            var go = quadChar.quadItem.gameObject;
            if (!go.activeSelf)
                go.SetActive(true);

            // finally, we set the quadGlyph rect & metrics
            // the 115.6 come from TMP EmojiOne SpriteGlyphTable
            float bearingY = m_quadRect.height / 128.0f * 115.6f;
            var glyphMetrics = new GlyphMetrics(m_quadRect.width, m_quadRect.height, 0, bearingY, m_quadRect.width);

            TMP_QuadGlyph quadGlyph = quadChar.glyph as TMP_QuadGlyph;
            if (quadGlyph != null)
            {
                quadGlyph.glyphRect = m_quadRect;
                quadGlyph.metrics = glyphMetrics;
                quadGlyph.scale = quadChar.gscale;
            }
            else
            { // glyph invalid, recreate it
                quadChar.SetQuadGlyph(new TMP_QuadGlyph((uint)index, glyphMetrics, m_quadRect, quadChar.gscale));
            }
            return quadChar;
        }

        /// <summary>
        /// Apply quad object transform
        /// </summary>
        /// <param name="i"></param>
        protected void ApplyQuadTransform(int i)
        {
            var charactorInfo = m_textInfo.characterInfo[i];
            if (charactorInfo.elementType == TMP_TextElementType.Quad)
            {
                var quadChar = (charactorInfo.textElement as TMP_QuadCharacter);
                var quadObject = quadChar.quadItem.gameObject;
                var quadTransform = (quadObject.transform as RectTransform);

                quadTransform.localPosition = CalculateQuadPosition(charactorInfo, !ignoreQuadScaleForVLayout ? 0 : quadChar.yoff);

                var quadMetrics = charactorInfo.textElement.glyph.metrics;
                var rscale = !ignoreQuadScaleForVLayout ? charactorInfo.scale : charactorInfo.scale * quadChar.gscale;
                var xscale = rscale;
                if (quadChar.yscale != 0 || quadChar.xscale != 0)
                    xscale = quadChar.xscale != 0 ? quadChar.xscale : 1.0f;
                quadTransform.sizeDelta = new Vector2(quadMetrics.width * xscale, quadMetrics.height * rscale);
            }
        }

        public void DeactiveAllQuadItem()
        {
            if (m_quadItems == null) return;
            for (int i = 0; i < m_quadItems.Count; ++i)
            {
                var quadItem = m_quadItems[i];
                if (quadItem != null && quadItem.gameObject.activeSelf)
                     quadItem.gameObject.SetActive(false);
            }
        }


        public void OnQuadRootDestroy(TMP_QuadRoot quadRoot)
        {
            for (int i = 0; i < m_quadItems.Count; ++i) {
                var quadItem = m_quadItems[i];
                if (quadItem != null)
                    DestroyQuadEntity(quadItem);
                m_quadItems[i] = null;
            }
        }

        /// <summary>
        /// Invoke quad entity to specific logic can release it's resources
        /// </summary>
        /// <param name="quadItem"></param>
        public void DestroyQuadEntity(TMP_QuadItem quadItem)
        {
            if(quadItem != null)
            {
                var entity = quadItem.Entity;
                if (entity != null)
                {
                    entity.OnDestroy();
                    quadItem.Entity = null;
                }
            }
        }

        protected Vector2 CalculateQuadPosition(TMP_CharacterInfo charactorInfo, int yoffset)
        {
            ...
            //maxAscender = Mathf.Max(maxAscender, charactorInfo.ascender);
            //minDescender = Mathf.Min(minDescender, charactorInfo.descender);

            //bottomLeft = new Vector3(charactorInfo.bottomLeft.x, charactorInfo.descender, 0);

            //// to world space
            //bottomLeft = rectTransform.TransformPoint(new Vector3(bottomLeft.x, minDescender, 0));
            //topRight = rectTransform.TransformPoint(new Vector3(charactorInfo.topRight.x, maxAscender, 0));

            //float width = topRight.x - bottomLeft.x;
            //float height = topRight.y - bottomLeft.y;

            //Vector3 worldPos = bottomLeft;
            //worldPos.x += width / 2; // centerX

            //// valign=bottom: (height + height / 4)
            //// valign=top: (-height / 2)
            //// valign=center: (height / 2)
            //// centerPosition.y += (height / 2); 

            //// to local space
            //var pos = rectTransform.InverseTransformPoint(worldPos);
            //pos.y = charactorInfo.bottomLeft.y + yoffset;

            //return pos;
            var pos = new Vector2(charactorInfo.vertex_BL.position.x + (charactorInfo.vertex_BR.position.x - charactorInfo.vertex_BL.position.x) / 2,
                charactorInfo.vertex_BL.position.y + yoffset);
            return pos;
        }
        #endregion
        ...
        /// </summary>
        /// <returns></returns>
        public Vector2 GetPreferredValues(float width, float height, bool enableWrap = false)
        {
            // Reparse input text
            ...
            // CALCULATE PREFERRED WIDTH
            float preferredWidth = GetPreferredWidth(margin, enableWrap);
        ...
        /// <param name="text"></param>
        /// <returns></returns>
        public Vector2 GetPreferredValues(string text, float width, float height, bool enableWordWrapping = false)
        {
            m_isCalculatingPreferredValues = true;
            ...
            // CALCULATE PREFERRED WIDTH
            float preferredWidth = GetPreferredWidth(margin, enableWordWrapping);
        ...
        /// <returns></returns>
        float GetPreferredWidth(Vector2 margin, bool enableWordWrapping = false)
        {
            float fontSize = m_enableAutoSizing ? m_fontSizeMax : m_fontSize;
            ...
            m_AutoSizeIterationCount = 0;
            float preferredWidth = CalculatePreferredValues(ref fontSize, margin, false, enableWordWrapping).x;
        ...
        protected virtual Vector2 CalculatePreferredValues(ref float fontSize, Vector2 marginSize, bool isTextAutoSizingEnabled, bool isWordWrappingEnabled)
        {
            ...
            // deal.g: fix preferred height calculate incorrect when TMP_Text used in TMP_InputField
            bool isEndsWithExtraSpace = totalCharacterCount > 0 && m_TextProcessingArray[totalCharacterCount - 1].unicode == 0X200B;
            int lastNonHoldCharIndex = !isEndsWithExtraSpace ? totalCharacterCount - 1 : totalCharacterCount - 2;
            ...
            for (int i = 0; i < m_TextProcessingArray.Length && m_TextProcessingArray[i].unicode != 0; i++)
            {
                ...
                if (m_textElementType == TMP_TextElementType.Character)
                {
                    if (/*(m_fontStyle & FontStyles.UpperCase) == FontStyles.UpperCase ||*/ (m_FontStyleInternal & FontStyles.UpperCase) == FontStyles.UpperCase)
                    {
                        ...
                        if (TMP_Char.IsLower(charCode))
                            charCode = TMP_Char.ToUpper(charCode);
                    ...
                    else if (/*(m_fontStyle & FontStyles.LowerCase) == FontStyles.LowerCase ||*/ (m_FontStyleInternal & FontStyles.LowerCase) == FontStyles.LowerCase)
                    {
                        ...
                        if (TMP_Char.IsUpper(charCode))
                            charCode = TMP_Char.ToLower(charCode);
                    }
                ...
                float elementHScale = 0;
                if(m_textElementType == TMP_TextElementType.Quad)
                {
                    ...
                    m_quadIndex = m_textInfo.characterInfo[m_characterCount].spriteIndex;

                    TMP_QuadCharacter quad = GetRefreshedQuadCharacter(m_quadIndex);
                    // Quad are assigned in the E000 Private Area + sprite Index
                    if (charCode == 60)
                        charCode = QUAD_CHARCODE_START + (uint)m_quadIndex;

                    #region compute quad scale,ascender,descender
                    // The quad scale calculations are based on the font asset assigned to the text object.
                    float fontScale = (m_currentFontSize / m_fontAsset.faceInfo.pointSize * m_fontAsset.faceInfo.scale * (m_isOrthographic ? 1 : 0.1f));
                    if (quad.yscale == 0 && quad.xscale == 0) {
                        currentElementScale = m_fontAsset.faceInfo.ascentLine / quad.glyph.metrics.height * quad.scale * fontScale;
                        elementHScale = currentElementScale * quad.glyph.scale;
                        if (!ignoreQuadScaleForVLayout) {
                            currentElementScale = elementHScale;
                        }
                    }
                    else
                        elementHScale = currentElementScale = quad.yscale != 0 ? quad.yscale : 1.0f;
                    
                    if (!ignoreQuadScaleForVLayout) {
                        float scaleDelta = fontScale / currentElementScale;
                        elementDescentLine = m_fontAsset.m_FaceInfo.descentLine * scaleDelta;

                        float diff = quad.m_Glyph.metrics.height - (m_fontAsset.m_FaceInfo.ascentLine - m_fontAsset.m_FaceInfo.descentLine);
                        elementAscentLine = m_fontAsset.m_FaceInfo.ascentLine + diff;

                        // adjust descent & ascent with yoff at here
                        float yoff = quad.yoff;
                        elementDescentLine += yoff;
                        elementAscentLine += yoff;
                    }
                    else {
                        float scaleDelta = fontScale / currentElementScale;
                        if (!ignoreQuadScaleForVLayout) scaleDelta *= quad.glyph.scale;
                        elementAscentLine = m_fontAsset.faceInfo.ascentLine * scaleDelta;
                        elementDescentLine = m_fontAsset.faceInfo.descentLine * scaleDelta;
                    }
                    #endregion

                    m_cached_TextElement = quad;

                    m_internalCharacterInfo[m_characterCount].elementType = TMP_TextElementType.Quad;
                ...
                else if (m_textElementType == TMP_TextElementType.Sprite)
                {
                    ...
                    else
                    {
                        ...
                        currentElementScale = m_fontAsset.faceInfo.ascentLine / sprite.glyph.metrics.height * sprite.scale * sprite.glyph.scale * spriteScale;
                        float scaleDelta = spriteScale / currentElementScale;
                        elementAscentLine = m_fontAsset.faceInfo.ascentLine * scaleDelta;
                        //baselineOffset = m_font.faceInfo.baseline * m_fontScale * m_fontScaleMultiplier * m_font.faceInfo.scale;
                        elementDescentLine = m_fontAsset.faceInfo.descentLine * scaleDelta;
                    }
                ...
                {
                    var normalChar = m_textInfo.characterInfo[m_characterCount].textElement;
                    if (normalChar == null) continue;
                    ...
                    currentElementScale = adjustedScale * m_fontScaleMultiplier * normalChar.scale;
                    ...
                    {
                        if (!TMP_Settings.useBestFit(charCode))
                        { // official logic: use fallback font asset ascentLine,descentLine
                            elementAscentLine = m_currentFontAsset.m_FaceInfo.ascentLine;
                            elementDescentLine = m_currentFontAsset.m_FaceInfo.descentLine;
                        }
                        else
                        { // by deal.g: use mainFont for bestFit chars
                            float fontScale = (m_currentFontSize / m_fontAsset.faceInfo.pointSize * m_fontAsset.faceInfo.scale * (m_isOrthographic ? 1 : 0.1f));
                            float scaleDelta = fontScale / currentElementScale;
                            elementAscentLine = m_fontAsset.faceInfo.ascentLine * scaleDelta;
                            elementDescentLine = m_fontAsset.faceInfo.descentLine * scaleDelta;
                    ...
                    m_cached_TextElement = normalChar;

                    m_internalCharacterInfo[m_characterCount].elementType = TMP_TextElementType.Character;
                }

                if (elementHScale == 0)
                    elementHScale = currentElementScale;
                #endregion
                ...
                if (charCode == 0xAD || charCode == 0x03)
                    elementHScale = currentElementScale = 0;
#endregion


                // Store some of the text object's information
                m_internalCharacterInfo[m_characterCount].character = charCode;
                ...
                bool isWhiteSpace = charCode <= 0xFFFF && TMP_Char.IsWhiteSpace(charCode);
...
#region Handle Kerning
                ...
                if (m_enableKerning)
                {
                    ...
                    m_internalCharacterInfo[m_characterCount].adjustedHorizontalAdvance = glyphAdjustments.xAdvance; // m_GlyphHorizontalAdvanceAdjustment = glyphAdjustments.m_XAdvance;
                }
#endregion
...
#region Handle Right-to-Left
                ...
                //    if (TMP_Char.IsWhiteSpace((char)charCode) || charCode == 0x200B)
...
#region Handle Mono Spacing
                ...
                if (m_monoSpacing != 0)
                {
                    monoAdvance = (m_monoSpacing / 2 - (m_cached_TextElement.glyph.metrics.width / 2 + m_cached_TextElement.glyph.metrics.horizontalBearingX) * elementHScale) * (1 - m_charWidthAdjDelta);
...
#region Handle Style Padding
                ...
                m_internalCharacterInfo[m_characterCount].origin = m_xAdvance + glyphAdjustments.xPlacement * elementHScale;
...
#region Handle Visible Characters
                if (charCode == 9 || (isWhiteSpace == false && charCode != 0x200B && charCode != 0xAD && charCode != 0x03) || (charCode == 0xAD && isSoftHyphenIgnored == false) || m_textElementType == TMP_TextElementType.Sprite || m_textElementType == TMP_TextElementType.Quad)
                {
                    ...
                    textWidth = Mathf.Abs(m_xAdvance) + currentGlyphMetrics.horizontalAdvance * (1 - m_charWidthAdjDelta) * (charCode == 0xAD ? currentElementUnmodifiedScale : elementHScale);
...
#region XAdvance, Tabulation & Stops
                ...
                {
                    m_xAdvance += ((currentGlyphMetrics.horizontalAdvance * m_FXScale.x + glyphAdjustments.xAdvance) * elementHScale + (m_currentFontAsset.normalSpacingOffset + characterSpacingAdjustment + boldSpacingAdjustment) * currentEmScale + m_cSpacing) * (1 - m_charWidthAdjDelta);
...
#region Check for Line Feed and Last Character
                if (charCode == 10 || charCode == 11 || charCode == 0x03 || charCode == 0x2028 || charCode == 0x2029 || m_characterCount == totalCharacterCount - 1)
                {
                    ...
                    if (charCode == 10 || charCode == 11 || (charCode == 0x2D && isInjectedCharacter) || charCode == 0x2028 || charCode == 0x2029)
                    {
                        ...
                        /* deal.g: fix preferred height incorrect when press character LF '\n' is last character */
                        if (charCode == 10 && (m_characterCount == lastNonHoldCharIndex))
                        {
                            m_RenderedHeight = m_maxTextAscender - (elementDescender - m_lineOffset)/*m_ElementDescender*/;
                        }
...
#region Save Word Wrapping State
                if (isWordWrappingEnabled || m_overflowMode == TextOverflowModes.Truncate || m_overflowMode == TextOverflowModes.Ellipsis)
                {
                    ...
                    if ((isWhiteSpace || charCode == 0x200B || charCode == 0x2D || charCode == 0xAD) && (!m_isNonBreakingSpace || ignoreNonBreakingSpace) && charCode != 0xA0 && charCode != 0x2007 && charCode != 0x2011 && charCode != 0x202F && charCode != 0x2060)
                    {
						...
						if ((charCode == 0x2D && m_characterCount > 0 && TMP_Char.IsWhiteSpace(m_textInfo.characterInfo[m_characterCount - 1].character)) == false)
                        {
            ...
            DestroyUnusedQuadEntities(quadCount);
...
#region Check Auto-Sizing (Upper Font Size Bounds)
        ...
        internal void InsertNewLine(int i, float baseScale, float elementHScale, float currentEmScale, float boldSpacingAdjustment, float characterSpacingAdjustment, float width, float lineGap, ref bool isMaxVisibleDescenderSet, ref float maxVisibleDescender)
        {
            ...
            float maxAdvanceOffset = (glyphAdjustment * elementHScale + (m_currentFontAsset.normalSpacingOffset + characterSpacingAdjustment + boldSpacingAdjustment) * currentEmScale + m_cSpacing) * (1 - m_charWidthAdjDelta);
...
#region Setup Normals & Tangents
        ...
        protected virtual void SaveQuadVertexInfo()
        {
            // Save the Vertex Position for the Character
#region Setup Mesh Vertices
            m_textInfo.characterInfo[m_characterCount].vertex_BL.position = m_textInfo.characterInfo[m_characterCount].bottomLeft;
...
#region
        ...
        protected void GetUnderlineSpecialCharacter(TMP_FontAsset fontAsset)
        {
            ...
            */
            ...
            if (character != null)
            {
                if (character.glyph.atlasIndex != 0) // @LoveEngine
                    Debug.LogWarning("The character uesd for underline must in first atlas of major SDF font, otherwise the underline <u> can't be draw properly.");
...
#region deal.g support fallback to os default font
            if (TMP_Settings.AllowFallbackOSFont)
            {
                bool cacheHit;
                character = TMP_Settings.GetCharacterFromOSFallbackFontAsset(unicode, fontStyle, fontWeight, out isUsingAlternativeTypeface, out cacheHit);
#if DEBUG
                string strCharacter = Char.ConvertFromUtf32((int)unicode);
                if (!cacheHit) // 只打印一次
                    UnityEngine.Debug.LogWarningFormat("The charactor <{0}> is missing , fallback to system font {1}", strCharacter, character != null ? "succeed." : "failed!");
#endif
                if (character != null)
                    return character;
            }
        ...
        public virtual string GetParsedText()
        {
            ...
            {
                buffer[i] = (char)m_textInfo.characterInfo[i].character;
            }
        ...
        internal bool ValidateHtmlTag(TextProcessingElement[] chars, int startIndex, out int endIndex)
        {
            ...
            for (int i = startIndex; i < chars.Length && chars[i].unicode != 0 && tagCharCount < m_htmlTag.Length && chars[i].unicode != '<'; i++)
            {
                ...
                if (attributeFlag == 1)
                {
                    if (tagValueType == TagValueType.None)
                    {
                        ...
                        else
                        {
                            ...
                            tagValueType = m_xmlAttribute[attributeIndex].valueType = TagValueType.StringValue;
                            m_xmlAttribute[attributeIndex].valueStartIndex = tagCharCount - 1;
                            m_xmlAttribute[attributeIndex].valueHashCode = TMP_TextParsingUtilities.UpdateHashCode(m_xmlAttribute[attributeIndex].valueHashCode, (char)unicode);
                            m_xmlAttribute[attributeIndex].valueLength += 1;
                        }
                    }
                    else
                ...
                if (attributeFlag == 0)
                    m_xmlAttribute[attributeIndex].nameHashCode = TMP_TextParsingUtilities.UpdateHashCode(m_xmlAttribute[attributeIndex].nameHashCode, (char)unicode);
...
#region Rich Text Tag Processing
            ...
            else
            {
                ...
                switch ((MarkupTag)m_xmlAttribute[0].nameHashCode)
                {
                    ...
                    case MarkupTag.SPRITE:
                        ...
                        return true;
                    case MarkupTag.QUAD: // deal.g <quad>
                        if (m_quadRect == null) m_quadRect = new GlyphRect();
                        m_textElementType = TMP_TextElementType.Quad;
    ...
    // deal.g: Helper check charactor since we use int to store charactor
    public class TMP_Char
    {
        public static bool IsControl(uint ch)
        {
            return ch <= 255 && char.IsControl((char)ch);
        }
        public static bool IsSeparator(uint ch)
        {
            return ch <= 255 && char.IsSeparator((char)ch);
        }
        public static bool IsLetterOrDigit(uint ch)
        {
            return ch <= 255 && char.IsLetterOrDigit((char)ch);
        }
        public static bool IsPunctuation(uint ch)
        {
            return ch <= 255 && char.IsPunctuation((char)ch);
        }
        public static bool IsWhiteSpace(uint ch)
        {
            return ch <= 255 && char.IsWhiteSpace((char)ch);
        }
        public static bool IsLower(uint ch)
        {
            return ch <= 255 && char.IsLower((char)ch);
        }
        public static bool IsUpper(uint ch)
        {
            return ch <= 255 && char.IsUpper((char)ch);
        }
        public static uint ToUpper(uint ch)
        {
            return ch <= 255 ? (uint)(char.ToUpper((char)ch)) : ch;
        }
        public static uint ToLower(uint ch)
        {
            return ch <= 255 ? (uint)(char.ToLower((char)ch)) : ch;
        }
    }
}
