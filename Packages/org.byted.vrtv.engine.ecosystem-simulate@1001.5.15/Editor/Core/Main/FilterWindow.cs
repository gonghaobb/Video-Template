...
namespace Matrix.EcosystemSimulate
{
    ...
    public class FilterWindow : EditorWindow
    {
        ...
        public class Element : IComparable
        {
            ...
            {
                return string.Compare(name, (o as Element)?.name, StringComparison.Ordinal);
            }
        }
        ...
        {
            public GUIStyle header = (GUIStyle) typeof(EditorStyles)
                .GetProperty("inspectorBig", BindingFlags.Static | BindingFlags.NonPublic)?.GetValue(null, null);
        ...
        private const int k_WindowHeight = 400 - 80;
        private const int k_HelpHeight = 80 * 0;
        private const string k_ComponentSearch = "NodeSearchString";

        private static Styles s_Styles;
        private static FilterWindow s_FilterWindow;
        ...
        private GroupElement m_ActiveParent
        {
            get { return m_Stack[m_Stack.Count - 2 + m_AnimTarget]; }
        }

        private Element[] m_ActiveTree
        {
            get { return m_HasSearch ? m_SearchResultTree : m_Tree; }
        }
        ...
        {
            get { return !Mathf.Approximately(m_Anim, m_AnimTarget); }
        }
        ...
        internal void OnGUI()
        {
            ...
            {
                string newSearch = (string) Invoke(typeof(EditorGUI), null, "SearchField", searchRect,
                    m_DelayedSearch ?? m_Search);
            ...
            if (evt.type == EventType.KeyDown)
            {
                ...
                if (!m_ActiveParent.HandleKeyboard(evt, s_FilterWindow, GoToParent))
                {
                    ...
                    if (evt.keyCode == KeyCode.DownArrow)
                    {
                        ...
                        m_ActiveParent.selectedIndex = Mathf.Min(m_ActiveParent.selectedIndex,
                            GetChildren(m_ActiveTree, m_ActiveParent).Count - 1);
        ...
        private const string k_SearchHeader = "Search";
        ...
        private void RebuildSearch()
        {
            ...
            foreach (var e in m_Tree)
            {
                ...
                string localName = e.name.ToLower().Replace(" ", "");
                ...
                for (int w = 0; w < searchWords.Length; w++)
                {
                    ...
                    if (localName.Contains(search))
                    {
                        // If the start of the item matches the first search word, make a note of that.
                        if (w == 0 && localName.StartsWith(search))
                            didMatchStart = true;
                    }
                    else
                    {
