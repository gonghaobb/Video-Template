...
Shader /*ase_name*/ "Hidden/Universal/PBR_Fog" /*end*/
{
	Properties
	{
		/*ase_props*/
	...
	SubShader
	{
		...
		*/
		...
		Pass
		{
			...
			uniform float4 _FogGrad;

			float VF_FOG_VERT(float3 wPos)
			{
				#if defined(CUSTOM_FOG) && defined(FOG_LINEAR)
				...
				float unityFogFactor = z * unity_FogParams.z + unity_FogParams.w;
			...
			float3 VF_FOG_FRAG(float3 col, float fogFac)
			{
				#if defined(CUSTOM_FOG) && defined(FOG_LINEAR)
				float3 newFac = lerp(_FogGrad, 1, fogFac); 
				return lerp(unity_FogColor.rgb, col.rgb, saturate(newFac));
				#else
				return col;
				#endif
			}
			...
			VertexOutput vert ( VertexInput v /*ase_vert_input*/ )
			{
				...
				half3 vertexLight = VertexLighting(vertexInput.positionWS, lwWNormal);
					...
					// CUSTOM_FOG
					half fogFactor = VF_FOG_VERT(vertexInput.positionWS);
				#else
					half fogFactor = 0;
				#endif
			...
			half4 frag ( VertexOutput IN /*ase_frag_input*/ ) : SV_Target
			{
				...
				half4 color = UniversalFragmentPBR(
					...
					// CUSTOM_FOG
					color.rgb = VF_FOG_FRAG(color.rgb, IN.fogFactorAndVertexLight.x);
				#endif
