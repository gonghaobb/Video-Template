...
Shader "PicoVideo/EcosystemSimulate/SunLightSimulate/LensFlare"
{
    Properties
    {
        ...
        Pass
        {
			...
			float GetOcclusion(float2 screenPos, float depth, float radius, float ratio)
			{
				...
				for (uint i = 0; i < DEPTH_SAMPLE_COUNT; i++)
				{
					...
					{
						float sampledDepth = LinearEyeDepth(tex2Dlod(_CameraDepthTexture, float4(pos, 0,0)).r, _ZBufferParams);
