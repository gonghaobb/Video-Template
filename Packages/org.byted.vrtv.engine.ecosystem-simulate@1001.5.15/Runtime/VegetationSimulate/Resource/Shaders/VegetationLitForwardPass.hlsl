...
#ifndef VEGETATION_FORWARD_LIT_PASS_INCLUDED
#define VEGETATION_FORWARD_LIT_PASS_INCLUDED
...
struct Attributes
{
    ...
    #if defined(_VEGETATION_LEAVES) || defined(_VEGETATION_TRUNK)
...
Varyings LitPassVertex(Attributes input)
{
    ...
    #if _VERTEX_WIND
        float3 offset;
        VegetationVertex(TransformObjectToWorld(input.positionOS.xyz), input.vertexColor, TransformObjectToWorldNormal(input.normalOS), offset);
    ...
    output.positionCS = vertexInput.positionCS;
        ...
        VegetationVertColorClip(output.positionWS, output.viewDirWS, output.normalWS, output.color.rgb);
    #endif
...
half4 LitPassFragment(Varyings input, half facing : VFACE) : SV_Target
{
    ...
    half4 color = VegetationFragmentLighting(inputData, surfaceData);
