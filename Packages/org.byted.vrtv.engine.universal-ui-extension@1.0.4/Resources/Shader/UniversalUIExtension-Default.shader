Shader "PICO Universal UI Extension/Default"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        CGPROGRAM
            ...
            void RadialFill2(float2 uv)
            {
                uv = uv - float2(0.5,0.5);
                // uv *= _RadialFillUVRatio;
                float anglePI = atan2(uv.x, uv.y);
                float angle01 = abs(step(0, 0.5) - frac(anglePI / UNITY_PI / 2 - 0));
                clip(angle01 - abs(0.5));
            }
