...
namespace Pico.Platform
{
    ...
    public static class CoreService
    {
        ...
        public static Task<PlatformInitializeResult> AsyncInitialize(string appId = null)
        {
            ...
            {
                Debug.LogWarning($"[PlatformSDK] AsyncInitialize Android ");
                AndroidJavaObject activity = new AndroidJavaClass("com.unity3d.player.UnityPlayer").GetStatic<AndroidJavaObject>("currentActivity");
                Debug.LogWarning($"[PlatformSDK] AsyncInitialize activity {activity}");
                var requestId = CLIB.ppf_InitializeAndroidAsynchronous(appId, activity.GetRawObject(), IntPtr.Zero);
                Debug.LogWarning($"[PlatformSDK] AsyncInitialize activity {activity} requestId : {requestId}");
                if (requestId == 0)
                {
