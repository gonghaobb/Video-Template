...
namespace Unity.XR.PXR
{
    public class PXR_OverlayManager : MonoBehaviour
    {
...
#endif
            PXR_Plugin.Render.UPxr_SetLayerBlend(true, new PxrLayerBlend
            {
                //透传-半透明混合修正方案2
                srcColor = PxrBlendFactor.PxrBlendFactorOne,
                dstColor = PxrBlendFactor.PxrBlendFactorOneMinusSrcAlpha,
                srcAlpha = PxrBlendFactor.PxrBlendFactorOne,
                dstAlpha = PxrBlendFactor.PxrBlendFactorOneMinusSrcAlpha
            });
        }
        ...
        private void BeginRendering(ScriptableRenderContext arg1, Camera[] arg2)
        {
            ...
            if (isMainCamera)
            {
                OnPreRenderCallBack(Camera.main);
            }
        }
        ...
        private void EndRendering(ScriptableRenderContext arg1, Camera[] arg2)
        {
            ...
            if (isMainCamera)
            {
                OnPostRenderCallBack(Camera.main);
            }
        }
                ...
                foreach (var compositeLayer in PXR_OverLay.Instances)
                {
                    ...
                    else if (compositeLayer.overlayShape == PXR_OverLay.OverlayShape.Cylinder)
                    {
                        ...
                        if (compositeLayer.modelScales[0].z != 0)
                        {
                            ...
                            // PICO VIDEO Modify
                            layerSubmit2.centralAngleLeft = compositeLayer.modelScales[0].x;
                            layerSubmit2.centralAngleRight = compositeLayer.modelScales[0].x;
                        }
                        else
                        {
                        ...
                        if (PXR_Plugin.Render.UPxr_SubmitLayerCylinder2(layerSubmit2))
                        {
                            ...
                            if (compositeLayer.modelScales[0].z != 0)
                            {
                                ...
                                // PICO VIDEO Modify
                                layerSubmit.centralAngle = compositeLayer.modelScales[0].x;
                            }
                            else
                            {
                                Debug.LogError("PXRLog scale.z is 0");
